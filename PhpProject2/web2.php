<?php
    ...
    if (isset($_POST['save'])) {
        ...
        if (!$uID) {
            $conn->query("INSERT INTO stars2 (rateIndex) VALUES ('$ratedIndex')");
            $sql = $conn->query("SELECT id FROM stars2 ORDER BY id DESC LIMIT 1");
            $uData = $sql->fetch_assoc();
            $uID = $uData['id'];
        } else
            $conn->query("UPDATE stars2 SET rateIndex='$ratedIndex' WHERE id='$uID'");

        exit(json_encode(array('id' => $uID)));
    }

    $sql = $conn->query("SELECT id FROM stars2");
    $numR = $sql->num_rows;

    $sql = $conn->query("SELECT SUM(rateIndex) AS total FROM stars2");
						...
						<div class="row fullscreen align-items-center justify-content-center">
							<div class="banner-content col-lg-6 col-md-12">
								...
								<p>
                                                                ...
                                                                <a href="https://www.bornfitness.com/the-every-persons-guide-to-immune-system-support/"><button class="primary-btn2 mt-20 text-uppercase ">More Information<span class="lnr lnr-arrow-right"></span></button></a>
							</div>
								
							<div class="col-lg-6 d-flex align-self-end img-right">
								<img class="img-fluid" src="img/stayf2.jpg" alt="">
							</div>
						</div>
				</div>
			</section>
